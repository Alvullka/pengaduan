...
@section('content')
    <div class="grid grid-cols-3">
        ...
        <div class="card glass" style="grid-column: span 1;">
            ...
            <p style="color: var(--text-muted); font-size: 0.875rem; margin-bottom: 1.5rem;">Sampaikan laporan Anda dengan detail dan jelas.</p>

            <form action="{{ route('complaints.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <label class="form-label" for="title">Judul Laporan</label>
                    <input type="text" id="title" name="title" class="form-control" required placeholder="Contoh: Fasilitas Kelas Rusak">
                </div>

                <!-- INPUT LOKASI -->
                <div class="form-group">
                    <label class="form-label" for="location">Lokasi Kejadian / Fasilitas</label>
                    <input type="text" id="location" name="location" class="form-control" required placeholder="Contoh: Lab Komputer 1, Toilet Lt. 2">
                </div>

                <div class="form-group">
                    <label class="form-label" for="description">Isi Laporan / Detail</label>
                    <textarea id="description" name="description" class="form-control" rows="5" required placeholder="Tuliskan detail kejadian atau keluhan Anda..."></textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <small style="color: var(--text-muted); font-size: 0.75rem; display: block; margin-top: 0.25rem;">Maksimal 2MB (JPG/PNG).</small>
                </div>
        ...
        <div class="card" style="grid-column: span 2;">
            <h2 style="font-size: 1.25rem; margin-bottom: 1.5rem;"><i class="bi bi-clock-history"></i> Riwayat Pengaduan Anda</h2>

            @if($complaints->isEmpty())
                <div style="text-align: center; padding: 3rem; background: #F9FAFB; border-radius: 8px; border: 1px dashed var(--border);">
            ...
            @else
                <div class="grid grid-cols-2" style="gap: 1rem;">
                    @foreach($complaints as $c)
                        <div style="padding: 1.5rem; border: 1px solid var(--border); border-radius: 8px; background: var(--surface); box-shadow: var(--shadow-soft);">
                            ...
                            <div style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 0.25rem;">
                                <i class="bi bi-geo-alt-fill" style="color: #ef4444;"></i> <strong>Lokasi:</strong> {{ $c->location }}
                            </div>
                            ...
                            @if($c->responses->count() > 0)
                                <div style="background: #f0fdf4; border-left: 3px solid #22c55e; padding: 0.75rem; font-size: 0.875rem; border-radius: 4px;">
                                    <strong style="color: #166534;">Tanggapan Petugas:</strong><br>
                                    <span style="color: #1f2937;">{{ $c->responses->last()->response }}</span>
                                </div>
                            @else
