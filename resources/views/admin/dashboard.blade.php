            ...
            <table>
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Nama Pelapor</th>
                        <th>Lokasi</th> <!-- TAMBAH HEADER LOKASI -->
                        <th>Judul & Laporan</th>
                        <th>Foto</th>
                        <th>Status</th>
                        <th>Aksi</th>
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span style="font-size: 0.875rem; font-weight: 500;">
                                    <i class="bi bi-geo-alt-fill" style="color: #ef4444;"></i> {{ $c->location }}
                                </span>
                            </td>
                            ...
                            <td>
                                ...
                                <div style="display:flex; flex-direction:column; gap:0.5rem; justify-content:flex-start; align-items:flex-start;">
                                    <!-- List Tanggapan -->
                                    @if($c->responses->count() > 0)
                                        <div style="width: 100%; margin-bottom: 0.5rem;">
                                            @foreach($c->responses as $resp)
                                                <div style="background: #fdfdfd; border-left: 3px solid var(--primary); padding: 0.5rem; margin-bottom: 0.25rem; font-size: 0.8rem; border-radius: 4px; position: relative;">
                                                    <strong>{{ $resp->user->username }}:</strong> {{ $resp->response }}
                                                    <form action="{{ route('responses.destroy', $resp->id) }}" method="POST" style="display:inline; margin-left: 5px;">
                                                        @csrf
                                                        <button type="submit" class="btn"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" style="text-align: center; color: var(--text-muted); padding: 2rem;">Belum ada pengaduan yang masuk.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
